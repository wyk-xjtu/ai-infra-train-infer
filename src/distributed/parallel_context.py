...
class ParallelContext:
    ...
    def init_distributed(
        ...
        设计说明:
        - 当 world_size == 1 时，跳过 dist.init_process_group（支持单卡调试）
        - TP进程组将连续的 tp_size 个 rank 划为一组
          例如 world_size=8, tp_size=2 → TP groups: [0,1], [2,3], [4,5], [6,7]
        - DP进程组将相同 tp_rank 的 rank 划为一组
          例如 world_size=8, tp_size=2, dp_size=4 → DP groups: [0,2,4,6], [1,3,5,7]

        Args:
            tp_size: 张量并行度（必须整除 world_size）
            dp_size: 数据并行度（tp_size * dp_size 必须等于 world_size）
        ...
        assert tp_size * dp_size == world_size, (
            f"tp_size({tp_size}) * dp_size({dp_size}) must equal world_size({world_size})"
        )
        ...
        if dp_size > 1:
            for tp_rank_idx in range(tp_size):
                ranks = [tp_rank_idx + dp_idx * tp_size for dp_idx in range(dp_size)]
                group = dist.new_group(ranks)
                if dist.get_rank() in ranks:
                    dp_group = group
